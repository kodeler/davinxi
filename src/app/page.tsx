    ...
    <div className="relative min-h-screen">
      ...
      <div className="absolute inset-0 flex py-2 md:py-4">
        <div className="flex flex-col justify-center px-2 md:px-4 w-1/4 md:w-1/5">
          {contents.filter((content) => content.side === 'left').map((content) => (
            <button
              key={content.id}
          ...
          {contents.map((content) => (
            <div
              ...
              className={`flex justify-center w-full absolute transition-opacity duration-500 ${
                activeContent === content.id ? 'opacity-100' : 'opacity-0 pointer-events-none'
              }`}
            >
              <div
                className={`flex flex-col ${
                  content.side === 'left' ? 'items-end mr-auto ml-2 md:ml-4' : 'items-start ml-auto mr-2 md:mr-4'
                } bg-black bg-opacity-80 p-4 md:p-8 rounded-lg shadow-lg max-w-md md:max-w-xl`}
              >
                <h2 className="text-gray-300 mb-2 text-sm md:text-base">{content.question}</h2>
                <p className="text-gray-300 mb-4 whitespace-pre-line text-sm md:text-base">{content.explanation}</p>
                <Image src={content.image} alt={content.title} width={300} height={225} className="mb-4 md:mb-8" />
              </div>
            </div>
          ))}
        </div>
